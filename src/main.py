        ...
        for absence in absence_list:
            ...
            with open('../html/individualized_attendance_report.html', 'r') as file:
                ...
                if school_info[school]['seat_time']['contact'] == 'OA':
                    html_email = file.read().replace('###table_data###', table_data).replace('###au###', str(au)).replace('###attendance_email###', school_info[school]['individualized_report_reply']).replace('###seat_time###', f'You can email any absent notes, doctor notes, etc., to hello@opportunitiesacademy.org. ').replace('###9absences###', '').replace('###attendance_policy###', "You can read more about OA Attendance Policy on page 28 of the OA Handbook.")
